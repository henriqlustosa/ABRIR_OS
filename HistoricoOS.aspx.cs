...
public partial class HistoricoOS : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!this.IsPostBack)
        {
            carregaGridHistoricoOS(login);
        }
    }


    private void carregaGridHistoricoOS(string login)
    {
        var lista = new List<SolicitanteDados>();
        lista = OsDAO.BuscarHistoricoOSPorLogin(login);
        gdvHistorico.DataSource = lista;
        gdvHistorico.DataBind();
    }
    protected void gdvHistorico_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        int idSolicitacao = Convert.ToInt32(gdvHistorico.DataKeys[Convert.ToInt32(e.CommandArgument)].Value.ToString());
